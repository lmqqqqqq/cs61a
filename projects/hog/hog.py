...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls > 0:
        ...
        sum = tail_points(opponent_score)
    return sum
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1, 11):
        ans = average_dice(i, dice) # must call dice() exact times or it may go wrong
        if ans > max:
            max = ans
            id = i
    return id
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    val = tail_points(opponent_score)
    if val >= threshold:
        return 0
    else:
        return num_rolls
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    val = square_update(0, score, opponent_score)
